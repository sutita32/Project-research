...
function StaticPersonBottom({typesNow, dataresearch , professorlist, sendTeacherIndex}) {
  ...
  if(typesNow === "จำแนกทั้งหมด"){
    for(let i=0;i<professorlist.length;i++){
      ...
      for(let j=0;j<dataresearch.length;j++){
        ...
        if(professorlist[i].Keyword === dataresearch[j].Keyword && typesNow === "จำแนกทั้งหมด") sum++;
      }
      list.push({
    ...
    for(let i=0;i<professorlist.length;i++){
      ...
      for(let j=0;j<dataresearch.length;j++){
        ...
        if(professorlist[i].Keyword === dataresearch[j].Keyword &&  dataresearch[j].ID_Type === 1) sum++;
      }
      list.push({
        ID :professorlist[i].ID_professor,
        name :professorlist[i].title_name+professorlist[i].firstname_professor+" "+professorlist[i].lastname_professor,
    ...
    for(let i=0;i<professorlist.length;i++){
      ...
      for(let j=0;j<dataresearch.length;j++){
        ...
        if(professorlist[i].Keyword === dataresearch[j].Keyword &&  dataresearch[j].ID_Type === 2) sum++;
      }
      list.push({
        ID :professorlist[i].ID_professor,
        name :professorlist[i].title_name+professorlist[i].firstname_professor+" "+professorlist[i].lastname_professor,
